...
$(document).ready(() => {
  ...
  var i = 1;
  let changeTitle = () => {
    let titles = ["a Web Developer", "a React Developer", "a NextJs Developer", "a Frontend developer", "a Programmer", "a Software Engineer", "an Author","an Entrepreneur","an OSS contributor", "a Mentor"];
    if (i == titles.length) {
      i = 0;
    }
...
function sendMail(e){
  ...
  mail_subject = document.getElementById('subject');
  mail_body = document.getElementById('message');

  console.log({mail_subject, mail_body});

  // window.open('mailto:test@example.com?subject=subject&body=body');
}

  //Changing Title Effect
  const c = baffle(".title1");
  c.set({
    characters: '█<▓ ░█▒█▒ ░▒/░█ ▓▒▓ █/██▒ █▓█/ █▒█ ░█▒█ ▓░█▒',
    speed: 120
  });
  ...
  var i = 1;
  let changeTitle1 = () => {
    let titles1 = ["is dynamic", "is creative", "is geeky", "is here"];
    if (i == titles1.length) {
      i = 0;
    }
    c.text(currentText => titles1[i]).reveal(1500);
    i++;
  }
  setInterval(changeTitle1, 3200);